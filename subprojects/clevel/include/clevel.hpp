 ...
 */
#define CLEVEL_CAS(ptr, oldval, newval)                                        \
...
template<typename Key, typename T, typename Hash = std::hash<Key>,
         typename KeyEqual = std::equal_to<Key>, size_t HashPower = 16>
class clevel_hash {

    template<typename ET>
    class big_ptr {
    public:
        persistent_ptr<ET> ptr;
        size_t padding[(256 - sizeof(ptr)) / sizeof(size_t)];
        void change_padding(int i, size_t j) { padding[i] = j; }
    };

    static void add_count_write(size_t size) {
#if defined(COUNTING_WRITE)
        /* The lock will be flushed back to PM, so we add a count here */
        pmhb_ns::sample_guard<clevel_hash<Key, T>, pmhb_ns::WRITE_COUNT>{size};
#endif
    }
...
public:
    ...
    using KV_entry_ptr_t = compound_pool_ptr<value_type>;

    using level_ptr_t = compound_pool_ptr<level_bucket>;

    using level_meta_ptr_t = compound_pool_ptr<level_meta>;
...
//    using scoped_t = shared_mutex_scoped_lock;
    ...
    struct ret {
        ...
        ret(pointer _entry, size_type _level_idx, difference_type _bucket_idx,
    ...
    ret generic_insert(const key_type &key, const void *param,
                       void (*allocate_KV)(pool_base &,
                                           ...
                                           const void *),
                       size_type thread_id, size_type id, size_t off);
    ...
    ret update(const value_type &value, size_type thread_id, size_t off = 0) {
        return generic_update(value.first, &value, allocate_KV_copy_construct,
                              thread_id, off);
    }

    ret update(value_type &&value, size_type thread_id, size_t off = 0) {
    ...
    ret generic_update(const key_type &key, const void *param,
                       void (*allocate_KV)(pool_base &,
                                           persistent_ptr<value_type> &,
                                           const void *),
                       size_type thread_id, size_t off = 0);
    ...
    void set_thread_num(size_type num) {
        if (thread_num > 0) {
            ...
            for (size_type i = 0; i < thread_num; i++) {
                ...
                if (tmp_entry[di].ptr != nullptr)
                    delete_persistent<value_type>(tmp_entry[di].ptr);
            }
        ...
        tmp_meta =
                make_persistent<persistent_ptr<level_meta>[]>(thread_num + 1);
        tmp_level =
                make_persistent<persistent_ptr<level_bucket>[]>(thread_num + 1);
        tmp_entry = make_persistent<big_ptr<value_type>[]>(thread_num + 1);
    }
    ...
    persistent_ptr<big_ptr<value_type>[]> tmp_entry;
    ...
    // double loadfactor() {
    //     auto total = 0ul, occupied = 0ul;
    //     for (auto i = m->last_level; i != m->first_level;
    //          i = i.get_address(my_pool_uuid)->up) {
    //         auto const &level = *i.get_address(my_pool_uuid);
    //         auto const &buckets = level.buckets;
    //         auto const bucket_num = level.capacity;
    //         total += bucket_num * assoc_num;
    //         for (auto j = 0ul; j < bucket_num; ++j) {
    //             auto const &bucket = buckets[j];
    //             auto const &slots = bucket.slots;
    //             for (auto k = 0ul;k<assoc_num; ++k) {
    //                 auto const &entry = slots[k];
...
#ifdef PMHB_LATENCY
    auto pmhb_guard = pmhb_ns::sample_guard<clevel_hash, pmhb_ns::SEARCH>{};
#endif
        ...
        do {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                if (f_b.slots[j].x.partial == partial &&
                    ...
                    if (key_equal{}(
                                f_b.slots[j].p.get_address(kv_pool_uuid)->first,
                                key)) {
                        return ret(f_b.slots[j].p.get_address(kv_pool_uuid), i,
                                   f_idx, j);
                    }
                }
            }
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                if (s_b.slots[j].x.partial == partial &&
                    ...
                    if (key_equal{}(
                                s_b.slots[j].p.get_address(kv_pool_uuid)->first,
                                key)) {
                        return ret(s_b.slots[j].p.get_address(kv_pool_uuid), i,
                                   s_idx, j);
                    }
                }
            }
...
void clevel_hash<Key, T, Hash, KeyEqual, HashPower>::del_dup(
    ...
    if (tmp1_u.x.partial == tmp2_u.x.partial) {
        ...
        if (e1.get_offset() == e2.get_offset()) {
            if (CLEVEL_CAS(&(p2->p.off), e2.raw(), 0)) {
                pop.persist(&(p2->p.off), sizeof(uint64_t));
                add_count_write(sizeof(uint64_t));
            }
        }

        // 2. Refer to different locations with the same contents
        else if (key_equal{}(e1.get_address(kv_pool_uuid)->first,
                             e1.get_address(kv_pool_uuid)->first)) {
            if (CLEVEL_CAS(&(p2->p.off), e2.raw(), 0)) {
                pop.persist(&(p2->p.off), sizeof(uint64_t));
                add_count_write(sizeof(uint64_t));

                PMEMoid oid = e2.raw_ptr(kv_pool_uuid);
#ifdef WRITE_KV
                pmemobj_free(&oid);
#endif
            }
        ...
        for (size_type i = 0; i < n_levels; i++) {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (f_e.get_offset() == 0) {
                    ...
                    continue;
                } else if (f_b.slots[j].x.partial != partial ||
                           !key_equal{}(f_e.get_address(kv_pool_uuid)->first,
                                        key)) {
                    continue;
                }
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (s_b.slots[j].x.partial != partial ||
                    !key_equal{}(s_e.get_address(kv_pool_uuid)->first, key))
                    continue;
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_insert(
        ...
        size_type thread_id, size_type id, size_t off) {
#ifdef PMHB_LATENCY
    auto pmhb_guard = pmhb_ns::sample_guard<clevel_hash, pmhb_ns::INSERT>{};
#endif
    // fmt::print("size {}\n", sizeof(value_type));
#ifdef INSERT_DEBUG
...
#endif
        // time_guard tg1("alloc");
        // persistent_ptr<value_type> ptr;
        // allocate_KV(pop, ptr, param);
        // tmp_entry[t_id].ptr = ptr;
        allocate_KV(pop, tmp_entry[t_id].ptr, param);
        add_count_write(sizeof(value_type));
        /* WRITE_KV */
    }
    KV_entry_ptr_u created(off ? off : tmp_entry[t_id].ptr.raw().off);
    ...
    RETRY_INSERT:
...
#endif
            if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
                delete_persistent_atomic<value_type>(tmp_entry[t_id].ptr);
                return ret(nullptr, level_num, 0, 0);
            } else if ((result == VACANCY_IN_LEFT ||
                        result == VACANCY_IN_RIGHT) &&
                       (level_num > 0 || !m->is_resizing)) {
                if (CLEVEL_CAS(&(e->off), old_e.raw(), created.p.raw())) {
        ...
        {
#ifdef INSERT_DEBUG
            time_guard tg1("expand", tg);
#endif
            fmt::print("expand in insert\n");
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::erase(const key_type &key,
                                                      size_type thread_id) {
#ifdef PMHB_LATENCY
    auto pmhb_guard = pmhb_ns::sample_guard<clevel_hash, pmhb_ns::DELETE>{};
#endif
        ...
        do {
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (tmp.x.partial == partial && tmp.p.get_offset() != 0) {
                    if (key_equal{}(tmp.p.get_address(kv_pool_uuid)->first,
                                    key)) {
                        if (CLEVEL_CAS(&(f_b.slots[j].p.off), tmp.p.off, 0)) {
                            pop.persist(&(f_b.slots[j].p.off),
                                        sizeof(uint64_t));
                            add_count_write(sizeof(uint64_t));
                            succ_deletion = true;

                            PMEMoid oid = tmp.p.raw_ptr(kv_pool_uuid);
#ifdef WRITE_KV
                            pmemobj_free(&oid);
#endif
                            ...
                            // before deletion's CLEVEL_CAS. Therefore, we can do context
            ...
            for (size_type j = 0; j < assoc_num; j++) {
                ...
                if (tmp.x.partial == partial && tmp.p.get_offset() != 0) {
                    if (key_equal{}(tmp.p.get_address(kv_pool_uuid)->first,
                                    key)) {
                        if (CLEVEL_CAS(&(s_b.slots[j].p.off), tmp.p.off, 0)) {
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::generic_update(
        ...
        void (*allocate_KV)(pool_base &, persistent_ptr<value_type> &,
                            const void *),
        size_type thread_id, size_t off) {

#ifdef PMHB_LATENCY
    auto pmhb_guard = pmhb_ns::sample_guard<clevel_hash, pmhb_ns::UPDATE>{};
#endif
    ...
    while (true) {
        ...
        if (result == FOUND_IN_LEFT || result == FOUND_IN_RIGHT) {
            ...
            if (succ_update && old_e == created.p) {
                ...
                return ret(true);
            } else if (CLEVEL_CAS(&(e->off), old_e.raw(), created.p.raw())) {
                pop.persist(&(e->off), sizeof(uint64_t));
                add_count_write(sizeof(uint64_t));
                // return ret(true);
                ...
                // find and before update's CLEVEL_CAS. Therefore, we can do
...
void clevel_hash<Key, T, Hash, KeyEqual, HashPower>::expand(
...
#ifdef PMHB_LATENCY
    auto pmhb_guard = pmhb_ns::sample_guard<clevel_hash, pmhb_ns::DOUBLE>{};
#endif
    ...
    if (cl->up == nullptr) {
        ...
        make_persistent_atomic<bucket[]>(pop, tmp_level[t_id]->buckets,
                                         new_capacity);
        add_count_write(sizeof(bucket) * new_capacity);

        pop.persist(tmp_level[t_id]->buckets);
        add_count_write(sizeof(tmp_level[t_id]->buckets));
        tmp_level[t_id]->capacity = new_capacity;
        pop.persist(tmp_level[t_id]->capacity);
        add_count_write(sizeof(tmp_level[t_id]->capacity));
        tmp_level[t_id]->up = nullptr;
        pop.persist(&(tmp_level[t_id]->up.off), sizeof(uint64_t));
        add_count_write(sizeof(uint64_t));

        // Append a new level.
        bool rc = CLEVEL_CAS(&(cl->up.off), 0, tmp_level[t_id].raw().off);
        ...
        while (true) {
            ...
            if (CLEVEL_CAS(&(meta.off), m_copy.off, tmp_meta[t_id].raw().off)) {
                pop.persist(&(meta.off), sizeof(uint64_t));
                add_count_write(sizeof(uint64_t));
                break;
            } else {
                ...
                if (cl->capacity >= new_capacity && m->is_resizing) {
                    // CLEVEL_CAS fails because other threads help updating meta
                    delete_persistent_atomic<level_meta>(tmp_meta[t_id]);
                    break;
                }
                // CLEVEL_CAS fails because other threads complete rehashing.
            }
        }
    } else {
        // Ohter threads finished expanding
        ...
        if (meta == m_copy) {
            ...
            while (true) {
                ...
                if (CLEVEL_CAS(&(meta.off), m_copy.off,
...
void clevel_hash<Key, T, Hash, KeyEqual, HashPower>::resize() {
    size_type thread_id = thread_num;
    difference_type t_id = static_cast<difference_type>(thread_id);
    // pin_thread(119);
    pool_base pop = get_pool_base();
    static size_t involved_kv = 0;
    // #ifdef PMHB_LATENCY
    //     auto pmhb_guard = pmhb_ns::sample_guard<clevel_hash, pmhb_ns::REHASH>{};
    // #endif
        ...
        {// timer start
            ...
            RETRY_REHASH:
                ...
                for (size_type slot_idx = 0; slot_idx < assoc_num; slot_idx++) {
                    ...
                    value_type *e = src_tmp.get_address(kv_pool_uuid);
                    ...
                    for (size_type j = 0; j < assoc_num; j++) {
                        ...
                        if (dst_tmp.get_offset() == 0) {
                            if (CLEVEL_CAS(&(dst_b1.slots[j].p.off),
                        ...
                        if (dst_tmp.get_offset() == 0) {
                            if (CLEVEL_CAS(&(dst_b2.slots[j].p.off),
                                           dst_tmp.raw(), src_tmp.raw())) {
                    ...
                    if (!succ) {
                        fmt::print("expand in resizing\n");
                        expand(pop, thread_id, m_copy);
                        // for (size_type j = 0; j < assoc_num; j++) {
                        //     KV_entry_ptr_t dst_tmp = dst_b1.slots[j].p;
                        //     fmt::print(
                        //             "key is {}\n",
                        //             dst_tmp.get_address(kv_pool_uuid)->first);
                        //     dst_tmp = dst_b2.slots[j].p;
                        //     fmt::print(
                        //             "key is {}\n",
                        //             dst_tmp.get_address(kv_pool_uuid)->first);
...
#ifdef PMHB_LATENCY
                if (involved_kv >= 1000) {
                    pmhb_ns::sample_guard<clevel_hash,
                                          pmhb_ns::RESIZE_ITEM_NUMBER>{
                            involved_kv};
                    involved_kv = 0;
                }
#endif
                ...
                add_count_write(sizeof(expand_bucket));
                if (static_cast<size_type>(expand_bucket) == bl->capacity) {
                    ...
                    while (true) {
                        ...
                        make_persistent_atomic<level_meta>(
                                pop, tmp_meta[t_id], m->first_level, bl->up,
                                levels_left != 2);
...
clevel_hash<Key, T, Hash, KeyEqual, HashPower>::get_key(KV_entry_ptr_t &e) {
    return e.get_address(kv_pool_uuid)->first;
}
...
template<typename Key, typename T, typename Hash, typename KeyEqual,
         size_t HashPower>
size_t clevel_hash<Key, T, Hash, KeyEqual, HashPower>::get_memory_usage() {
    while (true) {
        level_meta_ptr_t m_copy(meta);
        level_meta *m = static_cast<level_meta *>(m_copy(my_pool_uuid));
        // Bottom-to-top search.
        ...
        do {
            li = next_li;
            level_bucket *cl = li.get_address(my_pool_uuid);
            sum += cl->capacity * sizeof(bucket);
            next_li = cl->up;
        } while (li != m->first_level);

        // Context checking.
        if (m_copy == meta) return sum;
    }// end while(true)
}
