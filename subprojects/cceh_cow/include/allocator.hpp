...
#ifndef ALLOC_CCEH_COW_H
#define ALLOC_CCEH_COW_H
...
#define LOG_FATAL_CCEH_COW(msg)                                                \
    std::cout << msg << "\n";                                                  \
    exit(-1)
#define LOG_CCEH_COW(msg) std::cout << msg << "\n"
...
Allocator::Allocator(const char *pool_name, size_t pool_size) {
    if (!std::filesystem::exists(pool_name)) {
        LOG_CCEH_COW("creating a new pool");
        ...
        if (pm_pool_ == nullptr) {
            LOG_FATAL_CCEH_COW("failed to create a pool;");
        }
    } else {
        LOG_CCEH_COW(
        ...
        if (pm_pool_ == nullptr) {
            LOG_FATAL_CCEH_COW("failed to open the pool");
        }
    }
}
...
void Allocator::Allocate(void **ptr, uint32_t alignment, size_t size,
    ...
    TX_ONABORT { LOG_FATAL_CCEH_COW("Allocate: TXN Allocation Error"); }
    TX_END
}
void Allocator::Allocate(PMEMoid *pm_ptr, uint32_t alignment, size_t size,
    ...
    if (ret) { LOG_FATAL_CCEH_COW("Allocate: Allocation Error in PMEMoid"); }
}
...
void Allocator::Persist(void *ptr, size_t size) {
    ...
    pmhb_ns::sample_guard<CCEH_COW<KV>, pmhb_ns::WRITE_COUNT>{size};
#endif
}
...
//     }
//     TX_ONABORT { LOG_FATAL_CCEH_COW("ZAllocate: TXN Allocation Error"); }
//     TX_END
// #else
//     posix_memalign(ptr, alignment, size);
//     memset(*ptr, 0, size);
...
void Allocator::ZAllocate(PMEMoid *pm_ptr, uint32_t alignment, size_t size) {
    ...
    if (ret) {
        ...
        LOG_FATAL_CCEH_COW("allocation error");
    }
}
