...
#define LOG_FATAL_CCEH(msg)                                                    \
    std::cout << msg << "\n";                                                  \
    exit(-1)
#define LOG_CCEH(msg) std::cout << msg << "\n"
...
Allocator::Allocator(const char *pool_name, size_t pool_size) {
    if (!std::filesystem::exists(pool_name)) {
        ...
        if (pm_pool_ == nullptr) { LOG_FATAL_CCEH("failed to create a pool;"); }
    } else {
        LOG_CCEH("opening an existing pool, and trying to map to same address");
        /* Need to open an existing persistent pool */
        pm_pool_ = pmemobj_open(pool_name, layout_name);
        if (pm_pool_ == nullptr) { LOG_FATAL_CCEH("failed to open the pool"); }
    }
}
...
void Allocator::Allocate(void **ptr, uint32_t alignment, size_t size,
    ...
    TX_ONABORT { LOG_FATAL_CCEH("Allocate: TXN Allocation Error"); }
    TX_END
}
void Allocator::Allocate(PMEMoid *pm_ptr, uint32_t alignment, size_t size,
    ...
    if (ret) { LOG_FATAL_CCEH("Allocate: Allocation Error in PMEMoid"); }
}
...
void Allocator::Persist(void *ptr, size_t size) {
    ...
    pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::WRITE_COUNT>{size};
#endif
}
...
//     }
//     TX_ONABORT { LOG_FATAL_CCEH("ZAllocate: TXN Allocation Error"); }
//     TX_END
// #else
//     posix_memalign(ptr, alignment, size);
//     memset(*ptr, 0, size);
...
void Allocator::ZAllocate(PMEMoid *pm_ptr, uint32_t alignment, size_t size) {
    ...
    if (ret) {
        ...
        LOG_FATAL_CCEH("allocation error");
    }
}
