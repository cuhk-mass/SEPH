...
struct log_entry {
    ...
    void Lock_log() {
        uint64_t temp = 0;
        while (!CCEH_CAS(&lock, &temp, 1)) { temp = 0; }
    }
...
struct Segment {
    ...
    void release_lock(PMEMobjpool *pop) {
        ...
        pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::WRITE_COUNT>{sizeof(rwlock)};
#endif
#else
        mutex.unlock();
#endif
...
struct Directory {
    ...
    bool Acquire(void) {
        bool unlocked = false;
        return CCEH_CAS(&lock, &unlocked, true);
    }

    bool Release(void) {
        bool locked = true;
        return CCEH_CAS(&lock, &locked, false);
    }
...
public:
    ...
    // static CCEH *open(std::filesystem::path pool_path, size_t pool_size,
    //                   size_t init_depth);
    int
...
// template<class KV>
// static CCEH<KV> *open(std::filesystem::path pool_path, size_t pool_size,
//                       size_t init_depth) {
//     CCEH<KV> *ret = nullptr;
//     if (std::filesystem::exists(pool_path)) {
//         auto pop = pmem::obj::pool<CCEH<KV>>::open(pool_path, "CCEH");
//         ret = pop.root().get();
//     } else {
//         auto pop =
//                 pmem::obj::pool<CCEH<KV>>::create(pool_path, "CCEH", pool_size);
//         pre_fault(pop.handle(), pool_size);
//         new (ret) CCEH<KV>(init_depth, pop.handle());
//     }
// }
...
int Segment<KV>::Insert(PMEMobjpool *pool_addr, std::string_view key,
    ...
    for (unsigned i = 0; i < kNumPairPerCacheLine * kNumCacheLine; ++i) {
        ...
        if (CCEH_CAS(&_[slot].key.offset, &LOCK, SENTINEL)) {
...
PMEMoid *Segment<KV>::Split(PMEMobjpool *pool_addr, size_t key_hash,
                            log_entry *log) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::REHASH>{};
#endif
    using namespace std;
...
#ifdef PMHB_LATENCY
    pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::RESIZE_ITEM_NUMBER>{involved_kv};
#endif

#ifdef PMEM
    Allocator::Persist<KV>(split, sizeof(Segment<KV>));
#endif
#ifdef PMEM
    Allocator::Persist<KV>(this, sizeof(Segment<KV>));
#endif
    return &log[log_pos].temp;
}

template<class KV>
CCEH<KV>::CCEH(int initCap, PMEMobjpool *_pool) {
...
template<class KV>
bool CCEH<KV>::Update(std::string_view key, std::string_view value,
                      c_ptr<typename KV::K_TYPE> k_ptr,
                      c_ptr<typename KV::V_TYPE> v_ptr) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::UPDATE>{};
#endif
    uint64_t key_hash;
...
template<class KV>
void CCEH<KV>::Recovery(PMEMobjpool *_pop) {
...
template<class KV>
void CCEH<KV>::TX_Swap(void **entry, PMEMoid *new_seg) {
    TX_BEGIN(pool_addr) {
...
template<class KV>
void CCEH<KV>::Directory_Doubling(int x, Segment<KV> *s0, PMEMoid *s1) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::DOUBLE>{};
#endif
...
template<class KV>
void CCEH<KV>::Lock_Directory() {
    while (!dir->Acquire()) { asm("nop"); }
}

template<class KV>
void CCEH<KV>::Unlock_Directory() {
    while (!dir->Release()) { asm("nop"); }
}

template<class KV>
void CCEH<KV>::Directory_Update(int x, Segment<KV> *s0, PMEMoid *s1) {
...
template<class KV>
int CCEH<KV>::Insert(std::string_view key, std::string_view value,
                     bool is_in_epoch) {
...
int CCEH<KV>::Insert(std::string_view key, std::string_view value,
                     ...
                     c_ptr<typename KV::V_TYPE> v_ptr) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::INSERT>{};
#endif
...
#endif
            s = target->Split(pool_addr, key_hash, log);
        }
        if (s == nullptr) { goto RETRY; }
        auto ss = reinterpret_cast<Segment<KV> *>(pmemobj_direct(*s));
        ss->pattern =
                ((key_hash >> (8 * sizeof(key_hash) - ss->local_depth + 1))
                 << 1) +
                1;
        Allocator::Persist<KV>(&ss->pattern, sizeof(ss->pattern));
...
template<class KV>
bool CCEH<KV>::Delete(std::string_view key, bool is_in_epoch) {
    if (!is_in_epoch) { return Delete(key); }
    return Delete(key);
}

template<class KV>
bool CCEH<KV>::Delete(std::string_view key) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::DELETE>{};
#endif
    uint64_t key_hash;
    key_hash = h(key.data(), strlen(key.data()));
    auto y = (key_hash & kMask) * kNumPairPerCacheLine;
...
template<class KV>
Value_t CCEH<KV>::Get(std::string_view key, bool is_in_epoch) {
    if (is_in_epoch) { return Get(key); }
    return Get(key);
}

template<class KV>
Value_t CCEH<KV>::Get(std::string_view key) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<CCEH<KV>, pmhb_ns::SEARCH>{};
#endif
    uint64_t key_hash;
    key_hash = h(key.data(), strlen(key.data()));
    auto y = (key_hash & kMask) * kNumPairPerCacheLine;
...
template<class KV>
size_t CCEH<KV>::get_memory_usage() {
