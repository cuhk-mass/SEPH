...
template<class KV>
void Persist(PMEMobjpool *pop, const void *addr, size_t len,
             bool dont_count = false) {
    pmemobj_persist(pop, addr, len);
#if defined(COUNTING_WRITE)
    if (dont_count == false)
        pmhb_ns::sample_guard<LevelHashing<KV>, pmhb_ns::WRITE_COUNT>{len};
#endif
}
...
public:
    ...
    uint8_t try_movement(PMEMobjpool *pop, uint64_t, uint64_t,
                         c_ptr<typename KV::K_TYPE>,
                         c_ptr<typename KV::V_TYPE>);
    uint64_t F_HASH(std::string_view key);
    uint64_t S_HASH(std::string_view key);

    LevelHashing(void);
    LevelHashing(size_t);
    ~LevelHashing(void);
    void display_size() {
        std::cout << "The Node size is " << sizeof(Node<KV>) << std::endl;
        std::cout << "The Entry size is " << sizeof(Entry<KV>) << std::endl;

        if (buckets[0]) { printf("0x%" PRIXPTR "\n", (uintptr_t) buckets[0]); }

        if (buckets[1]) { printf("0x%" PRIXPTR "\n", (uintptr_t) buckets[1]); }
    }

    int Insert(std::string_view, std::string_view,
               c_ptr<KV> p_kv = c_ptr<KV>(0ul));
    int Insert(std::string_view key, std::string_view value, bool is_in_epoch) {
        return Insert(key, value);
    }

    bool Update(std::string_view key, std::string_view value, c_ptr<KV> pkv);
    bool Delete(std::string_view);
    bool Delete(std::string_view key, bool is_in_epoch) { return Delete(key); }
    Value_t Get(std::string_view);
    Value_t Get(std::string_view key, bool flag) { return Get(key); }
    void Recovery(PMEMobjpool *_pop) {
        c_ptr<typename KV::K_TYPE>::pool_uuid_lo =
                pmemobj_oid(_pop).pool_uuid_lo;
        c_ptr<typename KV::V_TYPE>::pool_uuid_lo =
                pmemobj_oid(_pop).pool_uuid_lo;
    ...
    void getNumber() {
        // std::cout << "Entry Size: " << sizeof(struct Entry<KV>) << std::endl;
        // std::cout << "Node Size: " << sizeof(struct Node<KV>) << std::endl;
        ...
        //           << (double) level_item_num[1] /
        //                      (addr_capacity * ASSOC_NUM / 2)
        //           << std::endl;
        std::cout << "load_factor: "
                  << (double) (level_item_num[0] + level_item_num[1]) /
                             (addr_capacity * ASSOC_NUM * 1.5)
                  << std::endl;
    }
    ...
    size_t get_memory_usage() {
        return (addr_capacity + addr_capacity / 2) * sizeof(Node<KV>) +
               sizeof(LevelHashing<KV>);
    }
};
...
template<class KV>
uint64_t LevelHashing<KV>::F_HASH(std::string_view key) {
    return h(key.data(), strlen(key.data()), f_seed);
}

template<class KV>
uint64_t LevelHashing<KV>::S_HASH(std::string_view key) {
    return h(key.data(), strlen(key.data()), s_seed);
}

template<class KV>
void LevelHashing<KV>::generate_seeds() {
    srand(time(NULL));
    do {
        f_seed = rand();
        s_seed = rand();
...
template<class KV>
LevelHashing<KV>::LevelHashing(void) {}
...
template<class KV>
void initialize_level(PMEMobjpool *_pop, LevelHashing<KV> *level, void *arg) {
    TX_BEGIN(_pop) {
        ...
        level->total_capacity =
                pow(2, level->levels) + pow(2, level->levels - 1);
        level->locksize = 128;
        ...
        level->_mutex = pmemobj_tx_zalloc(sizeof(PMEMrwlock) * level->nlocks,
                                          TOID_TYPE_NUM(char));
        level->_buckets[0] =
                pmemobj_tx_zalloc(sizeof(Node<KV>) * level->addr_capacity + 1,
                                  TOID_TYPE_NUM(char));
        level->_buckets[1] = pmemobj_tx_zalloc(
                sizeof(Node<KV>) * level->addr_capacity / 2 + 1,
                TOID_TYPE_NUM(char));
        level->_old_mutex = OID_NULL;

        /* Intialize pointer*/
        level->buckets[0] =
                (Node<KV> *) cache_align(pmemobj_direct(level->_buckets[0]));
        level->buckets[1] =
                (Node<KV> *) cache_align(pmemobj_direct(level->_buckets[1]));
...
template<class KV>
void remapping(LevelHashing<KV> *level) {
    level->buckets[0] =
            (Node<KV> *) cache_align(pmemobj_direct(level->_buckets[0]));
    level->buckets[1] =
            (Node<KV> *) cache_align(pmemobj_direct(level->_buckets[1]));
...
template<class KV>
int LevelHashing<KV>::Insert(std::string_view key, std::string_view value,
                             c_ptr<KV> pkv) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<LevelHashing<KV>, pmhb_ns::INSERT>{};
#endif
#ifdef INSERT_DEBUG
...

    {
        ...
        auto alloc_ret = pmemobj_alloc(pop, &p_kv, 40, 40, construct,
                                       (void *) &construc_args);
        pkv.offset = p_kv.off;
#endif
    }
    auto p_k = c_ptr<typename KV::K_TYPE>(pkv.offset + sizeof(pkv));
    auto p_v = c_ptr<typename KV::V_TYPE>(pkv.offset + sizeof(pkv) + 16);
RETRY:
    while (resizing_lock.load() == 1) { asm("nop"); }
    ...
    for (int i = 0; i < 2; ++i) {
        for (int j = 0; j < ASSOC_NUM; ++j) {
            if constexpr (std::is_pointer_v<KV>) {
                if ((buckets[i][f_idx].token[j] == 1) &&
                    (buckets[i][f_idx].slot[j].key->data() == key)) {
                    inserted = true;
                    goto UNIQUE;
                }
            } else {
        ...
        for (int j = 0; j < ASSOC_NUM; ++j) {
            if ((buckets[i][s_idx].token[j] == 1) &&
                (buckets[i][s_idx].slot[j].key->data() == key)) {
                inserted = true;
                goto UNIQUE;
            }
        }
...
UNIQUE:
    ...
    for (i = 0; i < 2; i++) {
        for (j = 0; j < ASSOC_NUM; j++) {
            while (pmemobj_rwlock_trywrlock(pop, &mutex[f_idx / locksize]) !=
                   0) {
                if (resizing == true) { goto RETRY; }
            }
            ...
            if (buckets[i][f_idx].token[j] == 0) {
                buckets[i][f_idx].slot[j].value = p_v;
                buckets[i][f_idx].slot[j].key = p_k;

                Persist<KV>(pop, &buckets[i][f_idx].slot[j], sizeof(Entry<KV>));
                buckets[i][f_idx].token[j] = 1;
                Persist<KV>(pop, &buckets[i][f_idx].token[j], sizeof(uint8_t),
                            1);
#ifdef COUNTING
                level_item_num[i]++;
#endif
            ...
            while (pmemobj_rwlock_trywrlock(pop, &mutex[s_idx / locksize]) !=
                   0) {
                if (resizing == true) { goto RETRY; }
            }
            ...
            if (buckets[i][s_idx].token[j] == 0) {
                buckets[i][s_idx].slot[j].value = p_v;
                buckets[i][s_idx].slot[j].key = p_k;
                Persist<KV>(pop, &buckets[i][s_idx].slot[j], sizeof(Entry<KV>));
                buckets[i][s_idx].token[j] = 1;
                Persist<KV>(pop, &buckets[i][s_idx].token[j], sizeof(uint8_t),
                            1);
#ifdef COUNTING
                level_item_num[i]++;
#endif
    ...
    if (resizing_lock.compare_exchange_strong(lock, 1)) {
        for (i = 0; i < 2; i++) {
            if (!try_movement(pop, f_idx, i, p_k, p_v)) {
                resizing_lock.store(0);
                return 0;
            }
            if (!try_movement(pop, s_idx, i, p_k, p_v)) {
                resizing_lock.store(0);
                return 0;
            }
            f_idx = F_IDX(f_hash, addr_capacity / 2);
        ...
        if (resize_num > 0) {
            {
                ...
                if (empty_loc != -1) {
                    buckets[1][f_idx].slot[empty_loc].value = p_v;
                    buckets[1][f_idx].slot[empty_loc].key = p_k;
                    Persist<KV>(pop, &buckets[1][f_idx].slot[empty_loc],
                                sizeof(Entry<KV>));
                    buckets[1][f_idx].token[empty_loc] = 1;
                    Persist<KV>(pop, &buckets[1][f_idx].token[empty_loc],
                                sizeof(uint8_t), 1);
#ifdef COUNTING
                    level_item_num[1]++;
#endif
            ...
            }
            {
                ...
                if (empty_loc != -1) {
                    buckets[1][s_idx].slot[empty_loc].value = p_v;
                    buckets[1][s_idx].slot[empty_loc].key = p_k;
                    Persist<KV>(pop, &buckets[1][s_idx].slot[empty_loc],
                                sizeof(Entry<KV>), 1);
                    buckets[1][s_idx].token[empty_loc] = 1;
                    Persist<KV>(pop, &buckets[1][s_idx].token[empty_loc],
                                sizeof(uint8_t), 1);
#ifdef COUNTING
                    level_item_num[1]++;
#endif
...
template<class KV>
void LevelHashing<KV>::resize(PMEMobjpool *pop) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<LevelHashing<KV>, pmhb_ns::DOUBLE>{};
#endif
    std::cout << "Resizing towards levels " << levels + 1 << std::endl;
    resizing = true;
    for (int i = 0; i < nlocks; ++i) { pmemobj_rwlock_wrlock(pop, &mutex[i]); }

    size_t new_addr_capacity = pow(2, levels + 1);
    _old_mutex = _mutex;
    Persist<KV>(pop, &_old_mutex, sizeof(_old_mutex));
    ...
    if (ret) {
        ...
        std::cout << "Allocation Size = " << std::hex
                  << nlocks * sizeof(PMEMrwlock) << std::endl;
    }
    mutex = (PMEMrwlock *) (pmemobj_direct(_mutex));
    ret = pmemobj_zalloc(pop, &_interim_level_buckets,
                         new_addr_capacity * sizeof(Node<KV>) + 1,
                         TOID_TYPE_NUM(char));
    if (ret) {
        std::cout << "Ret = " << ret << std::endl;
        std::cout << "Allocation Size = " << std::hex
                  << new_addr_capacity * sizeof(Node<KV>) + 1 << std::endl;
    }
    interim_level_buckets =
            (Node<KV> *) cache_align(pmemobj_direct(_interim_level_buckets));

    uint64_t new_level_item_num = 0;
    uint64_t old_idx;
    for (old_idx = 0; old_idx < pow(2, levels - 1); old_idx++) {
        uint64_t i, j;
        uint64_t involved_kv = 0;
        for (i = 0; i < ASSOC_NUM; i++) {
            if (buckets[1][old_idx].token[i] == 1) {
                auto key = buckets[1][old_idx].slot[i].key;
                auto value = buckets[1][old_idx].slot[i].value;

                uint32_t f_idx = F_IDX(F_HASH(key->data()), new_addr_capacity);
                uint32_t s_idx = S_IDX(S_HASH(key->data()), new_addr_capacity);
...
#ifndef BATCH
                        Persist<KV>(pop, &interim_level_buckets[f_idx].slot[j],
                                    sizeof(Entry<KV>));
#endif
                        interim_level_buckets[f_idx].token[j] = 1;
#ifndef BATCH
                        Persist<KV>(pop, &interim_level_buckets[f_idx].token[j],
                                    sizeof(uint8_t));
#endif
                        insertSuccess = 1;
#ifdef COUNTING
...
#ifndef BATCH
                        Persist<KV>(pop, &interim_level_buckets[s_idx].slot[j],
                                    sizeof(Entry<KV>));
#endif
                        interim_level_buckets[s_idx].token[j] = 1;
#ifndef BATCH
                        Persist<KV>(pop, &interim_level_buckets[s_idx].token[j],
                                    sizeof(uint8_t));
#endif
                        insertSuccess = 1;
#ifdef COUNTING
                ...
                Persist<KV>(pop, &buckets[1][old_idx].token[i],
                            sizeof(uint8_t));
#endif
                involved_kv++;
            }
...
#ifdef PMHB_LATENCY
        pmhb_ns::sample_guard<LevelHashing<KV>, pmhb_ns::RESIZE_ITEM_NUMBER>{
                involved_kv};
#endif
    }

#ifdef BATCH
    Persist<KV>(pop, &buckets[1][0], sizeof(Node<KV>) * pow(2, levels - 1));
    Persist<KV>(pop, &interim_level_buckets[0],
                sizeof(Node<KV>) * new_addr_capacity);
#endif
    ...
    TX_BEGIN(pop) {
        ...
        buckets[1] = (Node<KV> *) cache_align(pmemobj_direct(_buckets[1]));
        buckets[0] = (Node<KV> *) cache_align(pmemobj_direct(_buckets[0]));
...
template<class KV>
uint8_t LevelHashing<KV>::try_movement(PMEMobjpool *pop, uint64_t idx,
                                       uint64_t level_num,
                                       c_ptr<typename KV::K_TYPE> key,
                                       c_ptr<typename KV::V_TYPE> value) {
    uint64_t i, j, jdx;
    {
        ...
        for (i = 0; i < ASSOC_NUM; i++) {
            auto m_key = buckets[level_num][idx].slot[i].key;
            auto m_value = buckets[level_num][idx].slot[i].value;
            uint64_t f_hash = F_HASH(m_key->data());
            uint64_t s_hash = S_HASH(m_key->data());
            ...
            for (j = 0; j < ASSOC_NUM; j++) {
                if (buckets[level_num][jdx].token[j] == 0) {
                    ...
                    Persist<KV>(pop, &buckets[level_num][jdx].slot[j],
                                sizeof(Entry<KV>));
                    buckets[level_num][jdx].token[j] = 1;
                    Persist<KV>(pop, &buckets[level_num][jdx].token[j],
                                sizeof(uint8_t));
                    buckets[level_num][idx].token[i] = 0;
                    Persist<KV>(pop, &buckets[level_num][idx].token[i],
                                sizeof(uint8_t));

                    buckets[level_num][idx].slot[i].value = value;
                    buckets[level_num][idx].slot[i].key = key;
                    Persist<KV>(pop, &buckets[level_num][idx].slot[i],
                                sizeof(Entry<KV>));
...
template<class KV>
int LevelHashing<KV>::b2t_movement(PMEMobjpool *pop, uint64_t idx) {
    c_ptr<typename KV::K_TYPE> key;
    c_ptr<typename KV::V_TYPE> value;
    ...
    for (i = 0; i < ASSOC_NUM; i++) {
        ...
        f_hash = F_HASH(key->data());
        s_hash = S_HASH(key->data());
        f_idx = F_IDX(f_hash, addr_capacity);
        s_idx = S_IDX(s_hash, addr_capacity);
        ...
        for (j = 0; j < ASSOC_NUM; j++) {
            ...
            if (buckets[0][f_idx].token[j] == 0) {
                ...
                Persist<KV>(pop, &buckets[0][f_idx].slot[j], sizeof(Entry<KV>));
                buckets[0][f_idx].token[j] = 1;
                Persist<KV>(pop, &buckets[0][f_idx].token[j], sizeof(uint8_t));
                buckets[1][idx].token[i] = 0;
                Persist<KV>(pop, &buckets[1][idx].token[i], sizeof(uint8_t));
#ifdef COUNTING
                level_item_num[0]++;
                level_item_num[1]--;
#endif
            ...
            if (buckets[0][s_idx].token[j] == 0) {
                ...
                Persist<KV>(pop, &buckets[0][s_idx].slot[j], sizeof(Entry<KV>));
                buckets[0][s_idx].token[j] = 1;
                Persist<KV>(pop, &buckets[0][s_idx].token[j], sizeof(uint8_t));
                buckets[1][idx].token[i] = 0;
                Persist<KV>(pop, &buckets[0][s_idx].token[j], sizeof(uint8_t));
#ifdef COUNTING
                level_item_num[0]++;
...
template<class KV>
Value_t LevelHashing<KV>::Get(std::string_view key) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<LevelHashing<KV>, pmhb_ns::SEARCH>{};
#endif
RETRY:
    while (resizing == true) { asm("nop"); }
    uint64_t f_hash = F_HASH(key);
    uint64_t s_hash = S_HASH(key);
    uint32_t f_idx = F_IDX(f_hash, addr_capacity);
    uint32_t s_idx = S_IDX(s_hash, addr_capacity);
    ...
    for (i = 0; i < 2; i++) {
        {
            while (pmemobj_rwlock_tryrdlock(pop, &mutex[f_idx / locksize]) !=
                   0) {
                if (resizing == true) { goto RETRY; }
            }
            ...
            for (j = 0; j < ASSOC_NUM; j++) {
                if (buckets[i][f_idx].token[j] == 1 &&
                    buckets[i][f_idx].slot[j].key->data() == key) {
                    pmemobj_rwlock_unlock(pop, &mutex[f_idx / locksize]);
                    return buckets[i][f_idx].slot[j].value->data();
                }
            }
            pmemobj_rwlock_unlock(pop, &mutex[f_idx / locksize]);
        }
        {
            while (pmemobj_rwlock_tryrdlock(pop, &mutex[s_idx / locksize]) !=
                   0) {
                if (resizing == true) { goto RETRY; }
            }
            ...
            for (j = 0; j < ASSOC_NUM; j++) {
                if (buckets[i][s_idx].token[j] == 1 &&
                    buckets[i][s_idx].slot[j].key->data() == key) {
                    pmemobj_rwlock_unlock(pop, &mutex[s_idx / locksize]);
                    return buckets[i][s_idx].slot[j].value->data();
                }
            }
            pmemobj_rwlock_unlock(pop, &mutex[s_idx / locksize]);
        }
...
template<class KV>
bool LevelHashing<KV>::Update(std::string_view key, std::string_view value,
                              c_ptr<KV> pkv) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<LevelHashing<KV>, pmhb_ns::UPDATE>{};
#endif
RETRY:
    while (resizing == true) { asm("nop"); }
    uint64_t f_hash = F_HASH(key);
    ...
    uint32_t f_idx = F_IDX(f_hash, addr_capacity);
    uint32_t s_idx = S_IDX(s_hash, addr_capacity);
    // auto p_k = c_ptr<typename KV::K_TYPE>(pmemobj_oid(pkv->key()).off);
    auto p_v = c_ptr<typename KV::V_TYPE>(pkv.offset + sizeof(c_ptr<KV>) + 16);
    int i = 0, j;
    ...
    for (i = 0; i < 2; i++) {
        {
            ...
            for (j = 0; j < ASSOC_NUM; j++) {
                if ((buckets[i][f_idx].token[j] == 1) &&
                    (buckets[i][f_idx].slot[j].key->data() == key)) {
                    buckets[i][f_idx].slot[j].value = p_v;
                    Persist<KV>(pop, &buckets[i][f_idx].slot[j].value,
                                sizeof(uint8_t));
                    pmemobj_rwlock_unlock(pop, &mutex[f_idx / locksize]);
                    return true;
                }
        ...
        }
        {
            ...
            for (j = 0; j < ASSOC_NUM; j++) {
                if ((buckets[i][s_idx].token[j] == 1) &&
                    (buckets[i][s_idx].slot[j].key->data() == key)) {
                    buckets[i][s_idx].slot[j].value = p_v;
                    Persist<KV>(pop, &buckets[i][s_idx].slot[j].value,
                                sizeof(uint8_t));
                    pmemobj_rwlock_unlock(pop, &mutex[s_idx / locksize]);
                    return true;
                }
...
template<class KV>
bool LevelHashing<KV>::Delete(std::string_view key) {
#ifdef PMHB_LATENCY
    auto g = pmhb_ns::sample_guard<LevelHashing<KV>, pmhb_ns::DELETE>{};
#endif
RETRY:
    while (resizing == true) { asm("nop"); }
    uint64_t f_hash = F_HASH(key);
    ...
    for (i = 0; i < 2; i++) {
        {
            ...
            for (j = 0; j < ASSOC_NUM; j++) {
                if ((buckets[i][f_idx].token[j] == 1) &&
                    ...
                    Persist<KV>(pop, &buckets[i][f_idx].token[j],
                                sizeof(uint8_t));
                    pmemobj_rwlock_unlock(pop, &mutex[f_idx / locksize]);
                    return true;
                }
        ...
        }
        {
            ...
            for (j = 0; j < ASSOC_NUM; j++) {
                if ((buckets[i][s_idx].token[j] == 1) &&
                    ...
                    Persist<KV>(pop, &buckets[i][s_idx].token[j],
                                sizeof(uint8_t));
                    pmemobj_rwlock_unlock(pop, &mutex[s_idx / locksize]);
                    return true;
                }
