...
if __name__ == "__main__":
    ...
    fig, axes = plt.subplots(2, 4)  # len(data)//2
    axes = axes.flatten()
    axes[-1].set_visible(False)
    fig.set_size_inches(textwidth, textwidth/3)
    ...
    for i, (name, lax, rax) in enumerate(zip(data.keys(), laxes, raxes)):
        ...
        for left, right in zip(brkx[:-1], brkx[1:]):
            ...
            liney = sheet[rehash][int(xyx)]
            xyy = sheet[rehash][x.between(xyx, xyx + 0.1)].iloc[0]
            rax.annotate(f"{interval/1e6:.0f}", (xyx if interval//1e6 != 117 else (right+2*left)/3, xyy/25), color=rcolor,
                         fontsize='medium', va='bottom', ha='center', zorder=100000)
        handles = lax.get_lines()[:1] + rax.get_lines()
    laxes[0].set_ylabel('Real-time Throughput (ops)', ha="right")
    raxes[3].set_ylabel('Real-time Rehashing (ops)', ha="left", y=-1)
    raxes[-1].set_visible(False)
    fig.subplots_adjust(hspace=-0.4)
    fig.legend(handles, ["Real-time Throughput", "Real-time Rehashing"],
               bbox_to_anchor=(0.86, 0.2), loc='lower center', ncol=1)
