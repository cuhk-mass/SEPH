...
public:
  ...
  varlen_kv(unsigned _key_size, std::string_view _key, unsigned _value_size = 0,
            std::string_view _value = "")
      : key_size(_key_size), value_size(_value_size) {
    size_t real_key_size = std::min(_key.size(), (size_t)key_size - 1);
    strncpy(key(), _key.data(), real_key_size);
    key()[real_key_size] = 0;
    size_t real_value_size = std::min(_value.size(), (size_t)value_size - 1);

    if (_value_size) {
      strncpy(value(), _value.data(), real_value_size);
