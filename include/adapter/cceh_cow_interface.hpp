#ifndef PMHB_ADAPTER_CCEH_COW_HPP
#define PMHB_ADAPTER_CCEH_COW_HPP
...
using cceh_cow_map_type = cceh_cow_ns::CCEH_COW<varlen_kv>;

struct cceh_cow : public bench_interface<cceh_cow_map_type> {
    using map_type = cceh_cow_map_type;

    map_type *do_open(config const &cfg, size_t kv_uulo) override {
        auto path = cfg.working_dir / "cceh_cow";
        std::filesystem::remove_all(path);
        // auto map = map_type::open(path, pmhb_ns::MAP_STRUCTURE_SIZE, 1ul << 8);
        // return map;


        cceh_cow_ns::Allocator::Initialize(path.c_str(),
                                           pmhb_ns::MAP_STRUCTURE_SIZE);
        auto map = reinterpret_cast<map_type *>(
                cceh_cow_ns::Allocator::GetRoot(sizeof(map_type)));
        new (map) map_type(1ul << 8, cceh_cow_ns::Allocator::Get()->pm_pool_);
#ifndef WRITE_KV
        cceh_cow_ns::c_ptr<varlen_kv::K_TYPE>::pool_uuid_lo = kv_uulo;
        cceh_cow_ns::c_ptr<varlen_kv::V_TYPE>::pool_uuid_lo = kv_uulo;
        cceh_cow_ns::c_ptr<varlen_kv>::pool_uuid_lo = kv_uulo;
        fmt::print("UULO is {:x}\n", kv_uulo);
#endif
        return map;
    }
    ...
    map_type *do_recover(config const &cfg) override {
        ...
        f.close();
        map->Recovery(cceh_cow_ns::Allocator::instance_->pm_pool_);
    ...
    void do_close(map_type *cceh_cow, config const &cfg) override {
        cceh_cow->~map_type();
    }
...
#ifndef WRITE_KV
        auto k_ptr =
                cceh_cow_ns::c_ptr<varlen_kv::K_TYPE>(pkv + sizeof(varlen_kv));
        auto v_ptr = cceh_cow_ns::c_ptr<varlen_kv::V_TYPE>(
                pkv + sizeof(varlen_kv) + sizeof(varlen_kv::K_TYPE));
#else
        auto k_ptr = cceh_cow_ns::c_ptr<varlen_kv::K_TYPE>(0ul);
        auto v_ptr = cceh_cow_ns::c_ptr<varlen_kv::V_TYPE>(0ul);
#endif
